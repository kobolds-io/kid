...
var epoch_ms: u64 = DEFAULT_EPOCH_MS;
/// Unique identifier for this instance of KID
var node_id: u11 = 0;
// Internal state is packed
// [40 bit timestamp ms | 13 bits sequence | 11 node_id ]
var state: atomic.Value(u64) = atomic.Value(u64).init(0);

pub fn configure(n_id: u11, options: KIDOptions) void {
    node_id = n_id;
...
pub fn generate() u64 {
    while (true) {
        ...
        const swapped = state.cmpxchgWeak(old, new_bits, .seq_cst, .seq_cst);
...
pub fn decode(id: u64) Decoded {
    ...
    return .{
        .timestamp_ms = @as(u64, fields.timestamp) + epoch_ms,
